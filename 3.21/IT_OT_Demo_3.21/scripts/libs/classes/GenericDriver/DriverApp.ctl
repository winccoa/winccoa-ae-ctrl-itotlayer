...
//--------------------------------------------------------------------------------
  ...
  protected ConnectionAlertState connectionState = ConnectionAlertState::Disabled; //!< The connection state
    ...
    /**
   * @brief Returns if the driver stores the device connection settings as JSON in the connection DP ".Address"
   * @return FALSE by default to keep backward compatibility for existing drivers
   */

    /**
   * @brief Decode the connection settings stored in ".Address" as JSON
   * @param sAddressJson  JSON string from connection dp ".Address"
   * @return Decoded mapping, empty mapping if JSON is empty/invalid
   */
  protected mapping decodeJsonAddressConfig(const string &sAddressJson)
  {
    mapping mConfig;
    ...
    mConfig = jsonDecode(sAddressJson);

    if (getType(mConfig) != MAPPING_VAR)
    {
      return makeMapping();
    }
    ...
    /**
   * @brief Returns the JSON key used inside ".Address" for a device detail property
   * @param sProperty  Device detail property key
   * @return Empty string if the property is not stored inside the JSON config
   */
  protected string getJsonAddressConfigKey(const string &sProperty)
  {
    return "";
  }
  protected bool useJsonAddressConfig()
  {
    return FALSE;
  }
  ...
  protected void postCreateDeviceDp(const string &sDeviceKey, const langString &lsName, const langString &lsLocation, const langString &lsDescription, const string &sAddress, const mapping &mAdditionalParams)
  {
    ...
    {
      string sKey   = mappingGetKey(mDeviceDetailSource, i);
      string sValue = mappingGetValue(mDeviceDetailSource, i);

      // In JSON address mode the connection settings are stored in ".Address" as JSON,
      // therefore only direct relative DPEs that are not part of the JSON config shall be initialized here.
      if (useJsonAddressConfig() && getJsonAddressConfigKey(sKey) != "")
      {
        continue;
      }

      // Store the relative elements without '[]' brackets with the values from the additional parameters
      if (regexpIndex("(\\.\\S+)\\[(\\S+)\\]", sValue) < 0)
      {
        mValues[sValue] = mAdditionalParams[sKey];
      }
    }
...
//    spLog.doLog(EB_createLogEntry(DRIVER_NAME, LOG_TYPE_CONFIGURATION, this.getDeviceId(sDeviceKey) + " configuration", "", (int)ConfigurationState::Pending, getCurrentTime(), "", "configChanged", makeMapping("$device", lsName)), TRUE, TRUE);
    ...
    int iDeviceIndex = abs(DRIVER_DEVICE_ID_OFFSET + (int)getDeviceId(sDeviceKey)); // Search for device index by name
       ...
       mapping mJsonAddressConfig;
    bool bJsonAddressLoaded = FALSE;

    for (int i = 1; i <= iLen ; i++)
    {
      string sKey = mappingGetKey(mDeviceAttr, i);
      string sDPE = mDeviceAttr[sKey];

      if (strpos(sDPE, ".AddInfo[") == 0) //e.g. .AddInfo[location]
      {
        string sAttribute = strrtrim(substr(sDPE, strpos(sDPE, "[") + 1), "]");

        mAddInfo[sAttribute] = mParams[sKey];

        mappingRemove(mDeviceAttr, sKey);
      ...
      {
        // In JSON address mode some properties are stored inside the connection dp ".Address" JSON
        if (useJsonAddressConfig())
        {
          string sJsonKey = getJsonAddressConfigKey(sKey);
          ...
          if (sJsonKey != "")
          {
            ...
            if (dpExists(sDriverConnectionDP + ".Address"))
            {
              dpGet(sDriverConnectionDP + ".Address", sAddressJson);
            }

            mJsonConfig = decodeJsonAddressConfig(sAddressJson);
            mJsonConfig[sJsonKey] = mParams[sKey];

            dynAppend(dsDPEs, sDriverConnectionDP + ".Address");
            daValues[dynlen(daValues) + 1] = jsonEncode(mJsonConfig);
            continue;
          }
        }
        ...
        if (dpExists(sDPE))
        {
          ...
          if (dbArrayIndexItem[i])
          {
            ...
            else
            {
              ...
              aTmp[iDeviceIndex] = mParams[sKey];
            }
          }
          else
          {
...
//     driverident generic , refstr empty
    ...
    for (int i = 1; i <= iLen; i++)
    {
      ...
      else
      {
        ...
        if (dbArrayIndexItem[i])
        {
         ...
         //Ahmed - fix S7 internal dp restructure
        if (strpos(sDPE, ".AddInfo") == 0) //relative DPE
        {
          sDPE = sEbDriverDP + sDPE;
        }
  ...
  private void deleteDeviceDp(const string &sDeviceKey, mapping mParams)
  {
    ...
    if (CONNECTION_TAG_NEEDED)
    {
      this.deleteConnectionTag(sDeviceKey);
    }
