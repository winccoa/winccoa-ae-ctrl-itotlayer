...
//  use default: public static const dyn_string dsCsvHeaders = makeDynString("active", "tag", "address", "type", "rate","min", "max", "archive", "unit", "format", "desc");
 ...
 //Ahmed - fix S7 internal dp restructure
   public static const mapping mDeviceDetailSource = makeMapping("ipaddress",      ".Address",
                                                                "deviceType",     ".Address",
                                                                "deviceRack",     ".Address",
                                                                DriverConst::SLOT,     ".Address",
                                                                "timeout",        ".Address",
                                                                "tsppExtras",     ".Address",
                                                                "protocolExtras", ".Address",
...
//   protected mapping mDeviceDetailSource = makeMapping("IP", ".IPAddress", "TYPE", ".ConnectionType", "RACK", ".Rack", "SLOT", ".Slot");
  ...
  public static shared_ptr<DriverAppS7> createPointer()
  {
    ...
    private mapping decodeAddressJson(const string &sAddressJson)
  {
    mapping mAddress;
    ...
    if (!mappingHasKey(mAddress, "IPAddress"))      mAddress["IPAddress"] = "";
    if (!mappingHasKey(mAddress, "ConnectionType")) mAddress["ConnectionType"] = 2;
    if (!mappingHasKey(mAddress, "Rack"))           mAddress["Rack"] = 0;
    if (!mappingHasKey(mAddress, "Slot"))           mAddress["Slot"] = 2;
    if (!mappingHasKey(mAddress, "TimeOut"))        mAddress["TimeOut"] = 5000;
    if (!mappingHasKey(mAddress, "TSPPExtras"))     mAddress["TSPPExtras"] = "";
    if (!mappingHasKey(mAddress, "ProtocolExtras")) mAddress["ProtocolExtras"] = "";

    return mAddress;
  }

  public mapping getDeviceDetailsFromAddressJson(const string &sDeviceKey)
  {
    string sAddressJson;
    mapping mAddress;

    dpGet(getDriverConnectionDp(sDeviceKey) + ".Address", sAddressJson);
    mAddress = decodeAddressJson(sAddressJson);

    return makeMapping("ipaddress",      mAddress["IPAddress"],
                       "deviceType",     mAddress["ConnectionType"],
                       "deviceRack",     mAddress["Rack"],
                       DriverConst::SLOT,     mAddress["Slot"],
                       "timeout",        mAddress["TimeOut"],
                       "tsppExtras",     mAddress["TSPPExtras"],
                       "protocolExtras", mAddress["ProtocolExtras"],
                       "connectionsAll", "");
  }
...
//--------------------------------------------------------------------------------
  ...
  protected string getJsonAddressConfigKey(const string &sProperty)
  {
    switch (sProperty)
    {
      ...
      case "deviceType":     return "ConnectionType";
      case "deviceRack":     return "Rack";
      case DriverConst::SLOT:return "Slot";
      case "timeout":        return "TimeOut";
      case "tsppExtras":     return "TSPPExtras";
      case "protocolExtras": return "ProtocolExtras";
      default:               return "";
    }
  }
   ...
   protected void postUpdateDeviceDp(const string &sDeviceKey, const langString &lsName, const langString &lsLocation, const langString &lsDescription, const string &sAddress, const mapping &mAdditionalParams)
  {
   // DebugN("adddddd", getDriverConnectionDp(sDeviceKey) + ".Address"); //[_S7_Con]System1:_SinumerikPLC1.Address
    ...
    jsonEncode(makeMapping("TSPPExtras",     mAdditionalParams["tsppExtras"],
                           "ConnectionType", mAdditionalParams["deviceType"],
                           "TimeOut",        mAdditionalParams["timeout"],
                           "Slot",           mAdditionalParams[DriverConst::SLOT],
                           "ProtocolExtras", mAdditionalParams["protocolExtras"],
                           "Rack",           mAdditionalParams["deviceRack"],
                           "IPAddress",      sAddress)));
