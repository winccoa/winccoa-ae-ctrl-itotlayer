 ...
 */
void updateConfiguration(string sNewConfiguration)
{
...
//DebugTN("mmmmm", getLastError(), dmLastCfg, dmNewCfg, jsonDecode(sNewConfiguration), sNewConfiguration);
...
//DebugTN("udpate device and tags", dmOldSetting, dmNewSetting);
  ...
  for (int iNew = dynlen(dmNewSetting); iNew > 0; iNew--)
  {
    ...
    {
      DebugTN("ignore device!!",dmNewSetting[iNew][KEY_MNSP_PROTOCOL]);
...
//DebugTN("device already created?", dynContains(dsIdChecked, dmNewSetting[iNew][KEY_MNSP_DATASOURCE]));
    if (dynContains(dsIdChecked, dmNewSetting[iNew][KEY_MNSP_DATASOURCE]) < 1) //for performance optimization
    {
      bool bFound = FALSE;
      ...
      {
//DebugTN("create dev", dmNewSetting[iNew]);
